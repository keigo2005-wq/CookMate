...
（4つの画面はタブで切り替える構成のため、レシピ検索画面を起点に、
いつでも相互に行き来できる）  

### 5.3.	画面イメージ
図１　ログイン画面  
<img width="800" alt="ログイン画面" src="docs/screenshots/01_login.png" />  

図２　ログイン直後のレシピ検索画面（サイドバーのニックネームは伏字にしている）  
<img width="800" alt="ログイン直後の画面" src="docs/screenshots/02_after_login.png" />  

図３　食材・自由記述・条件を入力して検索した画面  
<img width="800" alt="レシピ検索画面" src="docs/screenshots/03_search_result.png" />  

図４　お気に入り画面  
<img width="800" alt="お気に入り画面" src="docs/screenshots/04_favorites.png" />  

図５　週間献立画面（今週の目標を入力し、AIに設計してもらった結果）  
<img width="800" alt="週間献立画面" src="docs/screenshots/05_meal_plan.png" />  

図６　マイレシピ画面  
<img width="800" alt="マイレシピ画面" src="docs/screenshots/06_myrecipes.png" />  
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>配色・角丸・影などのデザインの指針</td>
    </tr>
    <tr>
      <td>Lucide, Material Symbols</td>
      <td>絵文字に代わるアイコン</td>
    </tr>
    <tr>
      <td>ホスティング</td>
...
### 10.4.	開発中に見つけて対応した問題
　AIの説明文とデータのズレ：AIに1週間分の献立を選ばせたとき、実際に選んだ料理とは違う料理名を、理由の説明文の中で挙げてしまうことがあった。「説明文に書く料理名は、実際に選んだものだけにする」という指示をAIへの依頼文により具体的に書き直すことで解消した。  
　画面の表示が更新されない不具合：AIが決めた献立が、画面に反映されないことがあった。調べると、入力欄（選択ボックス）が一度表示されると、その時点の内容を覚え続けてしまい、後からプログラム側で内容を書き換えても上書きされてしまう、という画面作成ツール特有の癖が原因だった。入力欄側の記録も同時に書き換えることで直した。  
　通信エラー：埋め込みモデルのダウンロード時と、データベースへの接続時に、それぞれ通信の安全確認（証明書の検証）に失敗するエラーが発生した。原因や通信先に応じて、それぞれ別の対応方法を選ぶことで解決した。  
　公開先の環境特有の制限：アプリを公開したサービスでは、保存したファイルが時間が経つと消えてしまう仕組みになっていた。そのままでは意味検索が使えなくなってしまうため、検索のたびに「データが無ければ自動で作り直す」処理を追加し、手動での準備が要らないようにした。  
　スマホ表示の崩れ：スマートフォンの画面幅で開くと、横に並んだ画像が画面からはみ出してしまう不具合があった。原因は、画面のレイアウトを組み立てる仕組みが、画像の縦横比を保とうとして、必要以上に大きく確保してしまうという、よくある癖だった。画像の最小サイズの指定を1つ変えるだけで直った。  

### 10.5.	食材データの精度向上
　当初は、食材ごとに「レシピで使う分量あたりの栄養価・価格」を1つの目安値として持たせていたため、同じ食材でもレシピによって使用量が違うと、栄養価や費用の計算が不正確になる課題があった。  
　文部科学省の食品成分データベースを参照し、食材109種類すべての栄養価（カロリー・PFC・ビタミンC/E・カルシウム・鉄）と実売価格を、100gあたりの標準値に作り直した。あわせて、「大さじ1」「1個」「5cm」のようなレシピの分量表記を、食材ごとにグラム数へ変換する処理を実装し、実際に使う分量から栄養価・費用を計算する方式に変更した。これにより、脂質の多い食材と少ない食材の違いが正しく反映されるようになり、計算した栄養価をもとに「ダイエット向き」「美容」タグを自動判定する仕組みも実現した。  

### 10.6.	デザインの一貫性
　当初は絵文字とはっきりした色使いのUIだったが、見た目に一貫性を持たせるため、Googleの「Material Design 3」の考え方（トーンを抑えた配色・統一された角丸・控えめな影）を取り入れて再設計した。あわせて、絵文字はすべて、オープンソースのアイコンセット「Lucide」とGoogleの「Material Symbols」による線画アイコンに置き換えた。  

## 11.	今後の改善案
### 11.1.	ネイティブアプリとしての配布
　現在はWebブラウザ上で動作するアプリであり、スマートフォンのホーム画面に追加すればアプリのように開けるが、あくまでブラウザ経由での利用である。将来的には、App Store・Google Playで配布できる、正式なネイティブアプリとしての開発を検討する。  
